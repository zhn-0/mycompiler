...
TAC *do_func(SYM *func, TAC *args, TAC *code)
{
	...
	tlab = mk_tac(TAC_LABEL, func, NULL, NULL);
	tbegin = mk_tac(TAC_BEGINFUNC, func, NULL, NULL);
	tend = mk_tac(TAC_ENDFUNC, func, NULL, NULL);
...
TAC *do_assign(SYM *var, EXP *exp)
{
	...
	if (var->type != SYM_VAR && var->type != SYM_ARRAY)
